...
// Pure-pursuit tracker. Owns path preprocessing/projection locally so the shared
...
class ControllerPurePursuit : public ArduroverController {
  ...
  private:
    ...
    void PreprocessPath(std::vector<Waypoint> path);
    ...
    double lookaheadMin_{0.7};
    double lookaheadMax_{2.0};
    double lookaheadBase_{0.5};
    double lookaheadKv_{0.5};
    double maxYawRate_{1.0};
    double latAccelMax_{1.5};
    double decel_{0.8};
    double pivotAngle_{0.5};
